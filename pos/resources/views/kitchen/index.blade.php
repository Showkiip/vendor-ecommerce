                            ...
                            <div class="card card-custom gutter-b bg-transparent shadow-none border-0">
                                <div class="card-header align-items-center  border-bottom-dark px-0">
                                    <div class="card-title mb-0">
                                        <h3 class="card-label mb-0 font-weight-bold text-body">Kitchen
                                        </h3>
                                    </div>
                                    <div class="icons d-flex">
                                        <a href="{{ route('kitchen.create') }}" class="ml-2">
                                            <span
                                            ...
                                            <table id="productTable" class="display ">
                                                ...
                                                <tbody class="kt-table-tbody text-dark">
                                                    ...
                                                    <tr class="kt-table-row kt-table-row-level-0">
                                                        <td>{{ $loop->index+1 }}</td>
                                                        <td>{{ $type->name }}</td>
                                                        <td>{{ $type->kitchentype }}</td>
                                                        <td>{{ $type->quantity[1].''.$type->quantity[0]}} </td>
                                                            ...
                                                            <div class="card-toolbar text-right">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-right"
                                                                    ...
                                                                    <a type="button" class="dropdown-item"
                                                                        ...
                                                                        onclick="show_modal({{ $type->id }})">Edit
                                                                        Kitchen
                                                                    </a>
                                                                    <form action="{{ route('kitchen.destroy',$type->id) }}" method="post">
                                                                        @csrf
                                                                        @method('delete')
...
<div class="modal fade text-left" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel110"
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <form action="{{ route('kitchen.updated') }}" method="POST">
                @csrf
                ...
                <div class="modal-body">
                    <div class="tab-pane fade show active" id="home-basic" role="tabpanel"
                        ...
                        <h6 class="text-body">
                            <input type="hidden" name="id" id="id"><input type="hidden" name="type_id"
                                id="type_id">
                            Product Name
                        </h6>
                        <fieldset class="form-group mb-3">
...
$.ajax({
type:"post",
url: "{{ route('kitchen.editing') }}",
data:{kitchen_id: id, _token: _token},
success: function(type)
{
    console.log(type);
$('#id').val(type.id);
$('#name').val(type.name);
$('#quantity').val(type.quantity[1]);
$('#unit').val(type.quantity[0]);
