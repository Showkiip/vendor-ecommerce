                            ...
                            <div class="card card-custom gutter-b bg-transparent shadow-none border-0">
                                <div class="card-header align-items-center  border-bottom-dark px-0">
                                    <div class="card-title mb-0">
                                        <h3 class="card-label mb-0 font-weight-bold text-body">Sale Report
                                        </h3>
                                    </div>
                        ...
                        <div class="col-12 ">
                            <div class="card card-custom gutter-b bg-white border-0">
                                <div class="card-body">
                                   ...
                                   <a href="#" class="btn btn-secondary" onclick="dailySale()">Daily Sales</a>
                                   <a href="#" class="btn btn-primary" onclick="weeklySale()">Weekly Sales</a>
                                   <a href="#" class="btn btn-info" onclick="monthlySale()">Monthly Sales</a>
                                    <div>
                                        <div class="table-responsive" id="printableTable">
                                            <table id="productUnitTable" class="display" style="width:100%">
                                                ...
                                                <tbody class="kt-table-tbody text-dark" id="weeksales">
                                                    @foreach ($sales as $sale)
                                                    <tr class="kt-table-row kt-table-row-level-0">
                                                        <td>{{ $loop->index+1 }}</td>
                                                        <td class="">{{$sale->sale_id }}</td>
                                                        <td class="">{{ $sale->created_at->format('d-M-Y')}}</td>
                                                       <?php
                                                              $total = $sale->grand_total - $sale->return_amount ;
                                                       ?>
                                                       <td>{{round($total)}}</td>
                                                        <td class="">{{ round($sale->receive) }}</td>
                                                           <td class="">{{ round($sale->return_amount) }}</td>
                                                        <td class="">{{ round($sale->change) }}</td>
                                                        <td>
                                                            <div class="card-toolbar  no-sort text-right">
                                                                ...
                                                                <button type="button"
                                                                    ...
                                                                    onclick="viewDetailOfSale({{ $sale->id }})">
                                                                    View
                                                                </button>
                                                                <!-- {{-- </div> --}} -->
                                                            </div>
                                                    ...
                                                    $total_receive = Sale::sum('receive');
                                                    $total_change = Sale::sum('change');
                                                    $total_return = Sale::sum('return_amount');
                                                    @endphp
                                                   <td colspan="3">Overall Total</td>
                                                   <td>{{ round($grand_total) }}</td>
                                                   <td>{{ round($total_receive) }}</td>
                                                    <td>{{round($total_return)}}</td>
                                                   <td>{{round($total_change)}}</td>
                                            </table>
                                        </div>
                                    </div>
                                </div>
            ...
            success: function(result)
            {
                ...
                if(len > 0)
                {
                    for(var i=0; i<len; i++)
                    {
                        ...
                        var receive = result['daysales'][i].receive;
                        var return_amount = result['daysales'][i].return_amount;
                        var change = result['daysales'][i].change;
                        ...
                        var tr_str = "<tr>"+
                                    ...
                                    "<td >"+parseInt(receive)+"</td>"+
                                     "<td >"+parseInt(return_amount)+"</td>"+
                                    "<td >"+parseInt(change)+"</td>"+
                                    "<td> <div class='card-toolbar text-right'><button type='button' class='btn-link btn pt-0 pb-0 dropdown-item' onclick='viewDetailOfSale("+edit+")'> View</button></div></td>"+
                                    "</tr>";
                            $('#weeksales').append(tr_str);
                    ...
                    var tr_str ="<tr>"+
                                ...
                                "<td >"+parseInt(result['totalreceiveday'])+"</td>"+
                                "<td >"+parseInt(result['return_amount'])+"</td>"+
            ...
            success: function(result)
            {
                ...
                if(len > 0)
                {
                    for(var i=0; i<len; i++)
                    {
                        ...
                        var receive = result['weeksales'][i].receive;
                           var return_amount = result['weeksales'][i].return_amount;
                        var change = result['weeksales'][i].change;
                    ...
                    var tr_str ="<tr>"+
                                ...
                                "<td >"+parseInt(result['totalreceiveweek'])+"</td>"+
                                  "<td >"+parseInt(result['return_amount'])+"</td>"+
            ...
            success: function(result)
            {
                ...
                if(len > 0)
                {
                    for(var i=0; i<len; i++)
                    {
                        ...
                        var receive = result['monthsales'][i].receive;
                           var return_amount = result['monthsales'][i].return_amount;
                        var change = result['monthsales'][i].change;
                    ...
                    var tr_str ="<tr>"+
                                    ...
                                    "<td >"+parseInt(result['totalreceivemonth'])+"</td>"+
                                     "<td >"+parseInt(result['return_amount'])+"</td>"+
